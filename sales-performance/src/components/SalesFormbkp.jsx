...
const applyProductSelect = ({
  ...
  parts[parts.length - 1] = productName;

  const updated = [...list];
  updated[index].products = parts.join(", ");
    ...
    return (
      ...
      e.pis?.some(
        (p) =>
          ...
          p.products?.toLowerCase().includes(q),
      ) ||
      e.sales?.some(
        (s) =>
                                ...
                                onClick={() => {
                                 ...
                                 sales: (e.sales || []).map((s) => ({
                                      ...
                                      products: Array.isArray(s.products)
                                        ...
                                        ? s.products.split(",").map((item) => {
                                            const match = item.match(/(.+)\((\d+)\)/);
                                            return match
                                              ? { name: match[1].trim(), qty: Number(match[2]) }
                                              : { name: item.trim(), qty: 0 };
                                          })
                                        : [],

                                      piConfirmDate: s.piConfirmDate
                                        ? s.piConfirmDate.toDate().toISOString().split("T")[0]
                                        : "",
                                    }))
                                  });
